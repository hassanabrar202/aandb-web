...
const Chat = () => {
    ...
    useEffect(() => {
        ...
        if(receiver?.user_id)
        {
        const unsubscribe = getListofMessages(receiver.user_id, setChats);
        return () => {
            unsubscribe();
        };
            }
    ...
    const getallchatsdata = async (receiverDetails) => {
        ...
        const unsubscribe = getListofMessages(receiverDetails.user_id, updateMessages);

        // Optionally, you can store the unsubscribe function and call it when you no longer need updates
        // For example, when the component is unmounted
        // useEffect(() => {
        //     return () => unsubscribe();
        // }, []);

        // If you want to stop listening for updates manually, you can call the unsubscribe function
        // unsubscribe();
    };
